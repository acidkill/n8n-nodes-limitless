	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				default: '',
				description:
					'Will return all entries beginning on a date in the given timezone (YYYY-MM-DD)',
				displayOptions: {
					show: {
						operation: ['getLifelogs', 'exportMarkdown', 'summarizeDay'],
					},
			...
			},
			{
				...
				description:
					'The prompt to use for summarization. Use {{$JSON.lifelogsText}} as a placeholder for the lifelogs text.',
				displayOptions: {
					show: {
						operation: ['summarizeDay'],
					},
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (operation === 'getLifelogs') {
				...
				const options = {
					...
					OptionsWithUri,
					'uri' | 'method' | 'auth' | 'form' | 'followRedirect' | 'agentOptions'
				> & {
					uri: string;
				...
				const options = {
					...
					OptionsWithUri,
					'uri' | 'method' | 'auth' | 'form' | 'followRedirect' | 'agentOptions'
				> & { uri: string; method: IHttpRequestMethods; qs: IDataObject };
				...
				if (!date) {
					throw new NodeOperationError(
						this.getNode(),
						'Date parameter is required for Summarize Day operation.',
					);
				}
				...
				try {
					...
					if (Array.isArray(responseData)) {
						...
						this.logger.warn(
							'Unexpected response format when fetching lifelogs for summarization.',
						);
						lifelogsArray = [];
					}
				} catch (error) {
				...
				if (!chatModelCredentialName) {
					throw new NodeOperationError(
						this.getNode(),
						'Chat Model credential name is required for Summarize Day operation.',
					);
				}

				try {
					const chatModelInstance = await (this.helpers as any).getChatModel(
						chatModelCredentialName,
						i,
